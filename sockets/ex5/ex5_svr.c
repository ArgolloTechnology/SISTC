...
int main(int argc, char *argv[]){
  ...
  while(1) {
    ...
    if(f == 0){
		...
		msg2_t res;
		char s[100];
		strcpy(s, "Marvin Ryan Cerqueira de Teive e Argollo");
		int nBytes = strlen(s);
		char nBytesString[16];
		sprintf(nBytesString, "%d", nBytes);
		strcpy(res.student_name, nBytesString);
		strcat(res.student_name, "\n");
		strcat(res.student_name, s);
		strcpy(res.text, msg.text);
		printf("%s%s",res.text,res.student_name);
